---
name: imagnx
description: Generate or edit images using the imagnx CLI (OpenAI gpt-image, Google Gemini "Nano Banana"). Use whenever the user wants to create or modify an image. Triggers include "generate", "create", "make", "draw", "render", "paint", "design", "illustrate", "picture", "logo", "icon", "thumbnail", "sketch", "art", and edit phrasings like "edit this photo", "remove the background", "change the sky", "swap the X", "make it more X". Also use when the user names an image model (gpt-image-1.5, gpt-image-2, nano-banana, gemini-2.5-flash-image), asks to "compare image models", or invokes /imagnx. SKIP when the user wants to describe, analyze, OCR, or extract text from an image (vision, not generation), or wants charts, plots, or data visualizations.
